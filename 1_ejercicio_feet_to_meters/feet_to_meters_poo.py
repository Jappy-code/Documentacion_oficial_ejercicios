...
class FeetToMeters:
    ...
    def __init__(self, root):
        ...
        ttk.Label(mainframe, textvariable=self.meters).grid(column=1, row=1, sticky=(W, E))

        ttk.Button(mainframe, text="Calculate", command=self.calculate).grid(column=2, row=2, sticky=W)

        ttk.Label(mainframe, text="Feet").grid(column=2, row=0, sticky=W)
        ttk.Label(mainframe, text="is equivalent").grid(column=0, row=1, sticky=E)
